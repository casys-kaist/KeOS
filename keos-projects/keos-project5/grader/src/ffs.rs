...
pub fn add_file() {
    ...
    keos::info!("Creating the file `add_file' on the root directory");
    let created = root.create("add_file", false).unwrap();
    ...
    let f = idempotent
        .open("add_file")
        .expect("Created file `add_file' must be present on the root directory.");

    let f = f
        .into_regular_file()
        .expect("Created file `add_file' must be a RegularFile");
...
pub fn ib() {
    ...
    keos::info!("Extending the size of `ib' to 48 KiB which uses all of direct blocks");
    file.write(11 * 0x1000, &buf).unwrap();

    keos::info!("Extending the size of `ib' to 52 KiB which uses first entry of indirect blocks");
    file.write(12 * 0x1000, &buf).unwrap();

    keos::info!("Extending the size of `ib' to 56 KiB which uses second entry of indirect blocks");
...
pub fn dib() {
    ...
    keos::info!(
        "Extending the size of `dib' to 2092 KiB which uses all of direct blocks and indirect blocks"
    );
    ...
    keos::info!(
        "Extending the size of `dib' to 2096 KiB which uses first entry of doubly indirect blocks"
    );
...
pub fn add_directory() {
    ...
    keos::info!("Creating the directory `add_directory' on the root directory");
    let created = root.create("add_directory", true).unwrap();
    ...
    let f = idempotent
        .open("add_directory")
        .expect("Created file `add_directory' must be present on the directory.");

    let d = f
        .into_directory()
        .expect("Created file `add_directory' must be a Directory");
...
pub fn file_in_dir() {
    ...
    let root_file = root
        .create("file_in_dir__file", false)
        .unwrap()
        .into_regular_file()
        .unwrap();
    ...
    let dir = root
        .create("file_in_dir__directory", true)
        .unwrap()
        .into_directory()
        .unwrap();

    assert_eq!(
        dir.open("file_in_dir__file").map(|_| ()),
        Err(KernelError::NoSuchEntry),
        "Opening `file_in_dir__file' in `file_in_dir__directory' must be fail as it was never created."
    );

    dir.create("file_in_dir__file", false)
        .expect("Creating `file_in_dir__file' in `file_in_dir__directory' must be succeed");

    let dir_file = dir
        .open("file_in_dir__file")
        .expect("Created file `file_in_dir__file' must be present on `file_in_dir__directory'.")
        .into_regular_file()
        .unwrap();
...
pub fn remove_file() {
    ...
    let file = root
        .create("remove_file", false)
        .unwrap()
        .into_regular_file()
        .unwrap();
    file.write(0, b"Enjoying KeOS FFS?").unwrap();
    file.writeback().unwrap();
    keos::info!("Created file `remove_file` into the root directory.");

    root.unlink("remove_file").unwrap();
    keos::info!("Removed file `remove_file' from the root directory.");
...
pub fn remove_dir() {
    ...
    drop(dir);
    keos::info!("Created directory `remove_dir` with `file` inside.");

    assert_eq!(
        root.unlink("remove_dir").map(|_| ()),
        Err(KernelError::DirectoryNotEmpty),
        "Removing `remove_dir' without removing all entities in the directory must fail."
    );

    let dir = root.open("remove_dir").unwrap().into_directory().unwrap();
    dir.unlink("file").unwrap();

    drop(dir);

    root.unlink("remove_dir")
        .expect("Deleting empty directory `remove_dir' must succeed.");
}
...
pub fn simple_elf() {
    ...
    assert_eq!(
        run_elf_regularfile(&file, "the_answer"),
        0x42,
