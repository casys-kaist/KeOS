...
pub use keos_project2::loader::elf;
pub mod dev;
pub mod pager;
...
impl kev::vm::VmState for VmState {
    ...
    fn setup_vbsp(
        ...
        let entry = guard.entry() as u64;
        guard.unlock();
        vbsp_generic_state.vmcs.write(Field::GuestRip, entry)?;
        vbsp_generic_state.vmcs.write(Field::GuestRsp, 0xa0000)?;
        let mut guard = vbsp_vcpu_state.pager.lock();
        vbsp_generic_state.gprs.rsi = guard
            .finalize_mem()
...
impl kev::vcpu::VCpuState for VcpuState {
    ...
    fn init_guest_state(&self, vmcs: &ActiveVmcs) -> Result<(), VmError> {
        let guard = self.pager.lock();
        vmcs.write(Field::Eptptr, {
            guard.ept_ptr().into_usize() as u64 | (3 << 3) | 6
        })?;
        guard.unlock();
