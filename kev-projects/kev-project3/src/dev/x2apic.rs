...
//! In interrupt handling, the Advanced Programmable Interrupt Controller (APIC)
...
//! the CPU in the cases of the local core interrupts.
//!
...
//! the system bus. APIC's primary objective is to process interrupts on
//! processors and forward them to designated cores, as well as facilitate
//! Inter-Processor Interrupt (IPI) communication in multicore environments.
//!
//! Over time, Intel has enhanced the APIC to newer versions such as xAPIC and
...
//! modify performance behavior through the use of MSR registers.
//!
//! #### LAPIC
//! Each core has a dedicated Local APIC assigned to it, which can generate
...
//! Local APIC, which is directly connected to the processor's cores, performs
...
//! disabled.
//!
//! The definition of how each interrupt behaves can be configured via the Local
...
//! the core's interrupt pin.
//!
//! #### I/O APIC
//! The I/O APIC is an Interrupt Controller that is responsible for delivering
...
//! generated by hardware or I/O devices to the core.
//!
//! The I/O APIC has a Redirection Table that maps incoming interrupts from each
...
//! APIC Timer modes are separated into three different modes for handling timer
//! interrupts.
//!
//! #### Periodic Mode and One-shot Mode
//! The Periodic Mode is a mode in which software (usually the operating system)
...
//! the APIC to insert a timer interrupt every 1ms.
//!
//! The One-shot Mode is similar to the Periodic Mode but requests the timer
...
//! APIC to insert a timer interrupt 1ms later.
//!
//! #### TSC-Deadline Mode
//! The TSC-Deadline Mode has a slightly different characteristic from the other
...
//! interrupt when it becomes 100050, which is 100000 + α. Like one-shot mode,
...
//! deadline for Local APIC's TSC Deadline Mode is done by setting MSR with
//! 0x6e0 to a timestamp for the next deadline.
//!
//! ## Tasks
//! In this project, you are requested to implement timer interrupt
...
//! occurs every tick to schedule threads in the guest operating system.
//!
...
//! interrupt vector into the MSR 0x832 (the MSR write is trapped to guest).
//!
                        ...
                        let entry = unsafe {
                            (core::ptr::read_unaligned(
                                p.gpa2hva(&generic_vcpu_state.vmcs, Gpa::new(0x467).unwrap())
                                    .unwrap()
                                    .into_usize() as *const u16,
                            ) << 4)
                                | (core::ptr::read_unaligned(
                                    p.gpa2hva(&generic_vcpu_state.vmcs, Gpa::new(0x469).unwrap())
                                        .unwrap()
                                        .into_usize()
                                        as *const u16,
                                ) & 0xf)
